...
I ran the simulation for $120$ times for $25$ different number of beds (from $5-30$) and $350$ patients.
...
In Figure 5, we see that utilization percentages at $11$ beds for $350$ patients annually is less than $70%$. Typically $0.85 (85 pc)$ is considered an optimal utilization rate for wards; this threshold is often cited as a balance between efficiency (having enough patients to justify the resources used) and capacity (having enough free beds to accommodate influxes of patients). However, this figure can vary greatly depending on context. In the stroke the patient's length of stay is determined by the stroke severity.
...
Let's first visualize the length of stay distribution from the `stroke simulation function` -- as explained in detail in @sec-ssf, this function simulates the inflow, bed occupation times, queue and exit of patients from the stroke ward. I ran this function with 25 beds and 30 beds, and 40 beds to `num_beds` argument. Let's view the results of the distribution of the length that this simulation did for different number of beds.
