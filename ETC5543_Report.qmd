...
For example our DES simulation model in this study consists of multiple components, including patients, beds, and resources. Each of these components interacts with one another in a complex way, as patients move through the ward, occupy beds, and utilize resources. Traditional analytic methods might simplify these interactions or aggregate them, potentially losing critical information about the system's behavior.
...
Let $Y_1, Y_2, ..., Y_n$ be the stroke categories for $n$ patients, where $n =350$. Each $Y_i$ is an independent random variable taking values from $1 \to k$, where $k=6$ as there are six stroke categories . The probability of $Y_i$ taking the value $j$ : $(1 ≤ j ≤ 6)$ is given by:
