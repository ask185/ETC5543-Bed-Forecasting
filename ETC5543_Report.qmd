...
author: "Aryan Sultan"
department: "Department of Econometrics and Business Statisitcs (Monash University), Stroke & Ageing REsearch (STAR), Department of Medicine, School of Clinical Sciences (Monash University), Department of Neurology, Monash Health"
...
-   severe (NIHSS) (probability = 0.151).
    ...
    Bed occupation times ranged from 1 day (very mild) to 4-10 days (severe)[^1]. The study explored 5-50 bed scenarios and considered an increase in patient arrival to 2500 in the future. Simulation was run for $2000$ replications, evaluating performance measures (e.g., percent of patients waiting, maximum queue length, and capacity utilization) with an initial 7-bed provision for 350 patients annually.

[^1]: The bed occupation times for each category of stroke were provided to me by my supervisor, Prof. Phan. I declare that I did not have access to the actual data to ascertain the bed occupation times for each stroke category. The sources from which the bed occupation times were extracted were not disclosed to me either.
...
In Australia in 2020 an estimated $445,087$ [^2]Australians aged 15 and over were grappling with the aftermath of stroke.@deloitte2023 In 2018, $1.3\%$ of the Australian population had experienced a stroke at *some point* in their lives @AIHW2023 In 2020, the stroke incidence occured every 19 minutes in Australia. @deloitte2023

[^2]: According to the Economic Impact of Stroke (Deloitte Australia - 2020) [The report can be found here.](https://www2.deloitte.com/au/en/pages/economics/articles/economic-impact-stroke-australia.html)
...
The primary objective of this project was to develop *discrete event simulation* (DES) model that accurately captures the dynamics of the stroke ward at the Monash hospital. The DES model will simulate the flow of patients as they arrive, wait in line (queue) for an available bed, receive treatment, and exit the resource (making the bed available for the next patient in the queue to use).
...
By offering this tool in a web-based format, a wider audience can benefit from the insights generated by our simulation model, thereby enhancing decision-making processes in healthcare settings.
...
To counter these limitations, the critical information on patient's length of stay (bed occupation times), stroke category probabilities were supplied by the supervisor for this project at Monash Health, Professor Thanh Phan based on his expertise. Moreover, in this simulation model I have run $2000$ simulations to attain the results. This process was time consuming and computationally intensive, but produced reliable results.
...
The stroke categories were assigned based on the stroke level probabilities. The total annual patient load considered for this analysis was \~ $350$. The performance metrics were computed for $350$ yearly patients as well as for the an estimated annual load of 2500 patients for the future.
...
In the development of the `stroke_simulation()` R function, I used OpenAI's ChatGPT (OpenAI, 2021) @OpenAI2021 as an assistive tool for part of the process. However, the majority of the work, including the design, implementation, and testing of the function, was carried out independently. For more detail on this please see the footnote.[^3]

[^3]: I acknowledge the use of \[1\] ChatGPT (<https://chat.openai.com/>) to \[2\] re-structure my stroke simulation code so that modularity in the code and the intended results could be acheived. I entered the following prompts in April 2023:
...
Hence `run_simulation` function[^4] was created in the process of simulation to run multiple simulations and compute the performance metrics such as the utilization rates and the percent of patients waiting and others for **different** number of beds -- I have defined a range of beds in the function to run simulations for range of beds (5 to 50) and compute the performance metrics for these range of beds in each individual simulation and then averaged the performance metrics across these simulations.

[^4]: I acknowledge the use of \[1\] ChatGPT (<https://chat.openai.com/>) to \[2\] help structure my code in such a way that the simulation and computation of metrics could be applied across the range of beds and number of simulations.

    -   \[3\] This function \[code of the function\] does not work currently, I want the performance metrics as computed inside the function to be applied to every bed in the range of beds defined in the function as well as the number of simulations.
