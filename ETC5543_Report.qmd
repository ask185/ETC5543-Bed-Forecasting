...
Under-utilization of beds in the hospitals lead to additional costs, conversely, a shortage of available beds is the primary cause of delays in emergency admission, surgery cancellations, and premature patient discharge [@rechel2010hospital].
...
In 2020, stroke was recorded as the underlying cause of $8,200$ deaths, accounting for $5.1\%$ of all deaths in Australia. Stroke was one of the $5$ leading causes of death in Australia -- on average, $22$ Australians died of stroke each day in 2020.[^2]
...
While, according to the *Victorian Population Health Survey* (2016) [^4], the prevalence of stroke among the adult Victorian population was 2.7% of the adult population, in Melbourne, the incidence and prevalence of stroke follow similar trends observed across Australia.
...
For the purpose of this project at the *Monash Medical Research institute*, I have used the **simulation modelling** grounded in queue theory to represent a queuing system within a hospital setting.

### *Objectives and Significance*

#### **Objectives** {#sec-objectives}

The primary objective of this project was to develop *discrete event simulation* (DES) model that accurately captures the dynamics of the stroke ward at the Monash hospital.

The main objective of DES was to simulate the flow of patients as they arrive, wait in line (queue) for an available bed, receive treatment, and exit the resource (making the bed available for the next patient in the queue to use).
...
Key parameters such as the arrival rates, bed occupation times and the stroke level probabilities which were acquired from a combination of historical data, hospital statistics and relevant literature, were incorporated into the model. @green2011queueing By effectively representing the complex interaction between these factors, this DES model is meant to provide valuable insights into the stroke ward's operations at the Monash Hospital.
...
The Shiny app enables users to interactively adjust the number of patients, beds as well as the bed occupation times for different categories of the stroke using slider inputs, allowing them to visualize a plot of the percentage of patients waiting based on the selected number of patients and beds as well as a plot of the utilization rates at the selected number of beds.

Furthermore, the users are able to see the optimal number of beds for the selected patient load. By offering this tool in a web-based format, a wider audience can benefit from the insights generated by the simulation model, thereby enhancing decision-making processes in healthcare settings.

### *Methodology*

The methodology employed in this analysis is based on Discrete Event Simulation (DES), a widely recognized and effective technique for modeling complex systems with stochastic elements.
...
To ensure the accuracy of the model and critical parameters, the relevant datal was collected from 2021 records of the stroke patients in the hospital. Most importantly, the critical information on patient's length of stay (bed occupation times), stroke category probabilities were supplied by the supervisor for this project at Monash Health, Professor Thanh Phan.

One of the other challenges I faced was that the computation times were long. DES can be computationally intensive when running multiple thousand simulation or under multiple scenarios. However, in this simulation model I have run $2000$ simulations to attain the results. This process was time consuming and computationally intensive.
...
-   assign stroke categories to patients in the simulation,
...
This function first generated a sequence of numbers from $5- 30$ -- the range as I defined inside the function -- which represents the range of number of beds to be simulated. For each simulation and number of beds, the `stroke_simulation` function was run with the given parameters to simulate the scenario of a stroke ward with a specific number of beds and a certain number of patients.

After the simulation was run, various performance metrics were calculated based on the output of the `stroke_simulation` function.
...
In summary, remarkably, it was observed that the complex interplay between the count of hospital beds, yearly patient influx, and bed occupancy durations, results in substantial effects on both patient wait times and bed utilization ratios.
