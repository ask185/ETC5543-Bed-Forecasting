...
subtitle: "ETC5543 Business Creative Activity Project"
author: "Author: Aryan Sultan Khan"
department: "Department of Econometrics and Business Statisitcs (Monash University)"
Unit: "ETC5543 - Business Creative Activity"
email: "akha0094@student.monash.edu"
project-url: " https://github.com/ask185/ETC5543-Bed-Forecasting/"
...
[Github page](https://github.com/ask185/ETC5543-Bed-Forecasting/) (https://github.com/ask185/ETC5543-Bed-Forecasting)

[Web-app](https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/) ([aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/](https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/))

### **Summary**

**The Known:** Geographical stroke unit care is a proven stroke therapy with expectation that the patients spend their acute hospital journey in this unit.

**The New:** This report describes discrete event simulation approach to estimating the number of beds in geographical stroke unit to service the unique characteristics of each health service.

**The Implications:** A web-based app is provided to enable clinicians and policy maker to perform this estimate.

### ***Abstract***:

#### **Introduction**: {#sec-introduction}

Geographical stroke units offer proven stroke therapy for ischemic and hemorrhagic stroke patients. Optimal care is achieved when patients are allocated to these wards. Access to stroke care units has been attributed to as one of the leading causes of declining death rates among stroke patients in Australia. Consequently, estimating the optimal number of beds for a stroke ward is a critical and a complex task.
...
The simulation model considered factors such as patient arrival rates, stroke severity probabilities, and bed occupation times. Patient arrival rate were estimated to be 1 patients/day (on average) based on an annual patient load of \~350, $(\frac {350} {365} = \sim 1)$ and considered an increase in the annual load of patients of up to 2500 with the arrival rate $(\frac {2500} {365} = \sim 7)$. The probabilities of stroke severity was in line with scale defined by the National Institute of Health Stroke Scale (NIHSS) as provided by Prof. Phan for this project:

-   very mild (NIHSS) (probability = 0.0570),

-   mild (NIHSS) (probability = 0.342),

-   mild-moderate (NIHSS) (probability = 0.217),

-   moderate (NIHSS) (probability = 0.136),

-   moderate-severe (NIHSS) (probability = 0.0971) and

-   severe (NIHSS) (probability = 0.151).

Bed occupation times ranged from 1 day (very mild) to 4-10 days (severe)[^1]. The study explored 5-50 bed scenarios and considered an increase in patient arrival to 2500 in the future.

[^1]: My supervisor, Prof. Thanh Phan provided the bed occupation time for each category of stroke. I would like to note that I did not have direct access to original data to verify these times nor am I aware of the source(s) from which information regarding the length of stay for patients is derived.

Simulation was run for $2000$ replications, evaluating performance measures (e.g., percent of patients waiting, maximum queue length, and capacity utilization) with an initial 7-bed provision for 350 patients annually.

I have developed a web-based app such that policy makers and clinicians can use it as policy support tool to estimate the number of bed needed in their hospitals. (it can be found [aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/](https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/))
...
Hospital capacity planning is crucial due to the increasing global population, and the subsequent demand for effective healthcare services. Key metrics, such as bed capacity and associated indices such as bed occupancy and the ratio of beds to population are used to determine the availability of inpatient care [@Ravaghi2020].
...
In Australia in 2020 an estimated $445,087$ Australians aged 15 and over were grappling with the aftermath of stroke In 2018, $1.3\%$ of the Australian population had experienced a stroke at *some point* in their lives @AIHW2023 In 2020, the stroke incidence occured every 19 minutes in Australia. @deloitte2023

In 2020, stroke was recorded as the underlying cause of $8,200$ deaths, accounting for $5.1\%$ of all deaths in Australia. On average, $22$ Australians died of stroke each day in 2020. @AIHW2023

Since 1980 the death rates for stroke have fallen by three-quarters and access to stroke care units has been attributed to as one of the leading factors for declining death rates in Australia. @AustraliasHealth2022

While, the prevalence of stroke among the adult Victorian population was 2.7%, @VictorianPopulationHealthSurvey2016, according to the Australian Institute of Health and Welfare, the age-standardized prevalence of stroke in Melbourne has been on the rise, highlighting the need for better access to specialized care for stroke patients.
...
For the purpose of this project at the *Monash Medical Research institute*, I used the **simulation modelling** grounded in queue theory to represent a queuing system within a system of Monash hospital.

### *Objectives and Significance*

#### **Objectives** {#sec-objectives}

The primary objective of this project was to develop *discrete event simulation* (DES) model that accurately captures the dynamics of the stroke ward at the Monash hospital. This DES model simulates the flow of patients as they arrive, wait in line (queue) for an available bed, receive treatment, and exit the resource (making the bed available for the next patient in the queue to use).
...
By effectively representing the complex interaction between these different factors - patient arrivals, stroke levels (categories), corresponding lengths of stay and associated probabilities - my DES model provides valuable insights into the stroke ward's operations at the Monash Hospital.

#### **Novel Features:** {#novelFeatures}

While simulation modeling in healthcare is not a novel concept, in this project, I have developed a user-friendly shiny web-app, accessible [here.](https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/) (https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/) to broaden the accessibility of DES model in the context of a stroke ward to wider audience.

The Shiny app enables users to interactively adjust the number of patients, beds as well as the length of stay (bed occupation times) for each different categories of the stroke using slider inputs, which allows users to visualize a plot of the percentage of patients waiting to be admitted into the ward based on the selected number of patients and beds. In addition the users can also view a plot of the utilization rates at the selected number of beds. Furthermore, the users are able to see the optimal number of beds for the selected patient load.
...
-   DES effectively models complex systems, such as a stroke ward, where elements like patient arrivals, stroke severity, and length of stay are inherently unpredictable and dynamic. Traditional deterministic queuing models may struggle to analyze such systems due to their inherent variability.

-   DES accurately represents individual components and their interactions within a system. For instance, in this study, the DES model captures the complex interactions as patients move through the ward and utilize bed resources, which traditional methods might oversimplify or aggregate, potentially losing key system information.

-   A significant advantage of DES is its flexibility. It allows for the incorporation of changes in system behavior, testing of various scenarios, and interventions. For instance, in the stroke simulation model, changes in patient population, length of stay, and stroke level probabilities can be easily and reliably accounted for.
...
In this project, information on the stroke severity levels, length of stay for each stroke category and the associated probabilities were provided by Prof. Phan.
...
To counter these limitations, the supervisor for this project, Prof Thanh Phan provided the critical information on patient's length of stay (bed occupation times), associated stroke probabilities and patient arrivals based on his expertise and knowledge. Moreover, in this simulation model I have run $2000$ simulations to attain the results. This process was time consuming and computationally intensive, but produced reliable results.
...
The stroke categories were assigned based on the stroke level probabilities. The total annual patient load considered for this analysis was \~ $350$. The performance metrics were computed for $350$ yearly patients as well as for the an estimated annual load of 2500 patients for the future. (The results for 2500 patient influx can be viewed on the [web-app;](https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/) (https://aryan-sultan-1120.shinyapps.io/App_for_stroke_simulation/) in this report the initial annual patient load as well as the initial-provision of beds are considered and discussed.)
...
-   Arrival rate: $\frac {350} {365} =\ 0.958\ \sim 1$ patient per day on average -- this means on average one patient arrives into the ward each day for this simulation.

-   Stroke level probabilities & bed occupation times see Table 1:

```{r echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, table.position='H'}
...
```{r figcat, fig.cap="Stroke categories, probabilities and bed occupation times", fig.align= 'center', fig.height=4, fig.width= 6, eval=TRUE, fig.pos="H"}
...
-   Arrival rate = 1 patient per day on average, therefore, $\lambda =1$ The exponential distribution has a probability density function (PDF) given by:
...
In this simulation model once the inter-arrival times and the stroke categories to the incoming patients had been assigned, the next step was to generate the length of stay of these patients based on their assigned stroke level/category.
...
-   the stroke categories (stroke levels).
...
Having defined these functions, the next step in the simulation process was to check the distribution of the length of stay of patients and patients arrival data.
...
```{r eval=TRUE, echo=FALSE, warning=FALSE, table.position='H'}
...
In Table 2 one can see the first six rows of the arrivals data, which shows the inter-arrival times of the patients, stroke categories these patients were assigned and their length of stay in the ward.
...
In Figure 3, the distribution of the patients in each category follows the stroke category probabilities and the highest number of patients -- 112 -- were assigned to the *mild* category of stroke.
...
In the development of the `stroke_simulation()` R function, I used OpenAI's ChatGPT (OpenAI, 2021) @OpenAI2021 as an aiding (assistive) tool for part of the process. However, I undertook the majority of the work, including the design of the function, implementation, and testing of the function, independently. For more detail on this please see Appendix.

*Simulation Process*

The `stroke_simulation` function simulates the arrival of `total_patients` at a stroke ward with a limited number of beds (`num_beds`).

Patient's inter-arrival times, and length of stay based on the stroke categories are used inside the function to begin the simulation process.
...
In summary, the mathematics behind the `stroke_simulation` function involves generating inter-arrival times, length of stay, and assigning stroke categories, as well as efficiently managing bed availability and patient queues.
...
In the development of the `run_simulation()` R function, I used OpenAI's ChatGPT (OpenAI, 2021) @OpenAI2021 as an aiding (assistive) tool for part of the process. However, I undertook the majority of the work, including the design of the function, implementation, and testing of the function, independently. For more detail on this please see Appendix.

In this `run_simulation` function, I essentially implement a *Monte Carlo* simulation, which repeatedly simulate the operation of a stroke ward with varying numbers of beds, and compute performance metrics for each scenario. This allows for the analysis of how these metrics might vary under different conditions.
...
The function `run_simulation` takes five arguments: `arrival_rate`, `bed_occupation_time`, `stroke_level_prob`, `total_patients`, and `num_simulations`.
...
In Figure 5, we see that **utilization percentages** at $10$ beds for $350$ patients annually is less than $70 \%$. Typically $80 \% - 85 \%$ is considered an optimal utilization rate for wards; this threshold is often cited as a balance between efficiency (having enough patients to justify the beds used) and capacity (having enough free beds to accommodate influxes of patients). @aiken2002hospital

In the stroke ward a patient's length of stay is determined by the stroke severity and patient's medical history as well as their health condition at the time of the occurrence of the stroke. Some patients may need to stay for longer periods of time in the ward of up to two weeks days to recover, in such instances, the bed remains occupied by a patient for a longer than expected period of time. Thus, it becomes critically important to ensure that there are enough beds available to accommodate patient influxes.

```{r figsix, fig.cap="Max. Queue length vs the number of beds", fig.align= 'center', fig.height=4, fig.width= 6, eval=TRUE, message=FALSE, warning=FALSE, fig.pos="H"}
...
The maximum queue length in my simulation represents the highest number of patients waiting to be admitted at any point during the simulation period. In my simulation for $350$ patients per year and $10$ beds in the ward (optimal number of beds per utilization percentages), a maximum queue length of $15$ (as seen in Figure 6) means that at the peak of demand during the year, there were $15$ patients waiting to be admitted. However, it is important to note that this doesn't mean that $15$ patients will be waiting at all times, meaning, this is the peak demand and does not necessarily reflect the average wait time across the year.
...
```{r figseven, fig.cap="Utilization percentage vs the number of beds", fig.align= 'center', fig.height=4, fig.width= 6, eval=TRUE, message=FALSE, warning=FALSE, fig.pos="H"}
ggplot(results_df, aes(x = num_beds, y = utilization)) +
  geom_point() +
  geom_smooth(method = "gam") +
  scale_x_continuous(breaks = seq(4, 52, by=2))+
...
For stroke_simulation function:

I acknowledge the use of ChatGPT (<https://chat.openai.com/>) to re-structure my stroke simulation code so that modularity in the code and the intended results could be acheived. I entered the following prompts in April 2023:

-   I have created the following code \[code for update_bed_occupation_times which is used inside the stroke simulation function\]. In this function I am subtracting the inter-arrival time from beds (numeric vector of length of number of beds) but I do not want this to give me negative values, I would like it to be zero when the subtraction results in negative values.

-   Now that I have created the update_bed_occupation_time function \[code\] and have created the list of the available beds using \[beds \<- update_bed_occupation_times(beds, interarrival_times\[i\])\], and I have also created a vector of available beds using \[available_beds \<- which(beds == 0)\] in a while loop, now how can I assign the bed as well as set the bed equal to the length of stay of the patient to whom the bed is assigned \[the entire code\].
...
For run_simulation Function:

I acknowledge the use of ChatGPT (<https://chat.openai.com/>) to help structure my code in such a way that the simulation and computation of metrics could be applied across the range of beds and number of simulations.

```         
- This function [code of the function] does not work currently, I want the performance metrics as computed inside the function to be applied to every bed in the range of beds defined in the function as well as the number of simulations.
```
