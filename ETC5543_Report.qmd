...
The study utilized stroke unit data, including arrival rates, bed occupation times, and stroke severity probabilities, to generate realistic scenarios for the simulation model. A simulation model showed that the initial 7-bed provision was inadequate for the annual patient load of \~ $350$, leading to prolonged wait times and queues. The percent of patients queuing was around \~ $19 \%$ for 7 beds, $15\%$ for 8 beds, \~ $8\%$ for 9 beds and \~ $5\%$ for 10 beds.
...
A shortage of available beds is the primary cause of delays in emergency admission, surgery cancellations, and premature patient discharge [@rechel2010hospital]. Conversely, under-utilization of beds in the hospitals leads to additional hospital costs.
...
The primary objective of this project was to develop *discrete event simulation* (DES) model that accurately captures the dynamics of the stroke ward at the Monash hospital ensuring that the complex interactions between patients and beds are analyzed so that the model could be used to assess an optimal number of beds needed at the stroke ward to ensure that the stroke patients receive optimum care.

In order to achieve these objectives an evaluation of different bed capacities on the percent of the patients waiting to be admitted into the ward and the utilization rates/percentages were considered. One of the goals was also to provide decision support tool to the hospital administrators and policymakers in optimizing bed allocation at the stroke ward.

#### **Significance and Contributions**: {#sec-significanceContribution}

In this project, *Discrete Event Simulation* has been employed to develop a queuing system that accurately captures the intricate complexity and inherent variability of a stroke ward. This model takes into account the stochastic nature of the patient arrivals, stroke severity levels, and length of stay, enabling a comprehensive analysis of the hospital ward's performance under different scenarios.
...
-   Arrival rate: $\frac {350} {365} =\ 0.958\ \sim 1$ patient per day
...
Let $Y_1, Y_2, ..., Y_n$ be the stroke categories for $n$ patients, where $n =350$. Each $Y_i$ is an independent random variable taking values from $1 \to k$, where $k=6$ is the number of stroke categories . The probability of $Y_i$ taking the value $j$ : $(1 ≤ j ≤ 6)$ is given by:
...
Let's denote $B_1, B_2, ..., B_k$ as the bed occupation times for each of the \$k\$ stroke categories, with $B_j$ corresponding to stroke category $j$. These are given by the \`bed_occupation_time\` list in the function.

The function `generate_length_of_stay` assigns a length of stay $L_i$ for each patient \$i\$, where $i = 1, 2, ..., n$ and $n$ is the total number of patients. $L_i$ is defined as follows:

If $B_{Y_i}$ has only one value, then $L_i = B_{Y_i}$.

If $B_{Y_i}$ has more than one value (let's say it's a range from $B_{Y_i}^{(min)}$ to $B_{Y_i}^{(max)}$), then $L_i$ is a random sample from the range $B_{Y_i}^{(min)}$ to $B_{Y_i}^{(max)}$.

Mathematically, this can be represented as:

$L_i = \begin{cases} B_{Y_i} & \text{if } B_{Y_i} \text{ has only one value} \\ \text{random sample from } B_{Y_i}^{(min)} \text{ to } B_{Y_i}^{(max)} & \text{if } B_{Y_i} \text{ has more than one value} \end{cases}$

Therefore, the `generate_length_of_stay` function constructs a vector $\mathbf{L} = (L_1, L_2, ..., L_n)$ representing the length of stay for each patient. Each $L_i$ depends on the assigned stroke category $Y_i$ of the patient, and follows the distribution of bed occupation times for that stroke category.

The key aspect here is that the function applies different distributions for the length of stay depending on the stroke category. Thus, the function captures the variability in bed occupation times across different stroke categories.
...
This way, the R function `generate_length_of_stay` computes the length of stay for each patient based on their stroke category using the piece-wise constant function described above.
...
In Figure: 2 one can see the mean length of the patients' stay in the stoke ward against the stroke categories/levels. This figure shows us that in the arrivals data the patients that were assigned the severe category stayed the longest in the ward, on average staying more than 9 days in the ward. We can further examine the distribution of patients in each of the stroke category.

In Figure 3, the distribution of the patients in each category follows the stroke category probabilities and the highest number of patients -- 121 -- were assigned to the *mild* category of stroke --refer to Table 1 for information on the categories and the respective probabilities.

```{r echo=FALSE, eval=TRUE, message=FALSE, warning=FALSE, fig.align='center', fig.cap="Distribution of the stroke patients", fig.width=6, fig.height=5}
...
In order to simulate the patient inflow, stay and discharge I created the `stroke_simulation` function, which uses the previously created functions to achieve the intended results.

*Simulation Process*

The `stroke_simulation` function simulates the arrival of `total_patients` patients at a stroke ward with a limited number of beds (`num_beds`). The inter-arrival times of the patients, their length of stay in the ward, and their stroke categories are utilized to begin the simulation process.

For each arriving patient, this function first updates the bed occupation times for all beds, considering the time that has passed since the last patient arrival. Keeping track of the available beds, this function checks if there are any available beds. Vacant beds are assigned to the patients in case of the bed availability, else the patients are added to the queue, if all the beds are in use by other patients.
...
For each arriving patient, the remaining bed occupation time for all beds is updated by subtracting the inter-arrival time since the last patient's arrival. If the remaining time becomes negative, it is set to 0 (using the `pmax()` function), indicating that the bed is available.

Mathematically, the updated bed occupation time can be represented as:

$Tn_i = max(T_i - t_{interarrival}, 0)$
...
As discussed in detail in section @sec-ssf the stroke simulation function allows for simulating the patient inflows, their stay in the ward, their eventual exit from the ward and creating a list of the patients queuing up to be admitted.

It is important to run the simulation multiple times to obtain more reliable results to ensure that the results generated from this model are not due to random chance. Each simulation run produces different results due to the inherent randomness in the model. Averaging the outcomes over the several runs produces more stable estimates of the true expected values.

Hence `run_simulation` function was created in the process of simulation to run multiple simulations and compute the performance metrics such as the utilization rates and the percent of patients waiting and others for **different** number of beds -- I have defined a range of beds in the function to run simulations for range of beds and compute the performance metrics for these range of beds in each individual simulation and then average performance metrics across these simulations.

In this `run_simulation` function, I essentially implement a *Monte Carlo* simulation, which is a mathematical technique that allows us to account for risk in quantitative analysis and decision making. Monte Carlo simulation performs risk analysis by building models of possible results by substituting a range of values---a probability distribution---for any factor that has inherent uncertainty. It then calculates results over and over, each time using a different set of random values from the probability functions.
...
This function first generated a sequence of numbers from $5- 30$ -- the range as I defined inside the function -- which represents the range of number of beds to be simulated, then a `lapply` function was used twice in a nested manner to apply the `stroke_simulation` function for each number of beds and for each simulation. The outer `lapply` was for the number of simulations and the inner `lapply` was for each number of beds.[^5] For each simulation and number of beds, the `stroke_simulation` function was run with the given parameters to simulate the scenario of a stroke ward with a specific number of beds and a certain number of patients. After the simulation was run, various performance metrics were calculated based on the output of the `stroke_simulation` function. These performance metrics were then stored in a data frame for each number of beds and each simulation, and returned as a list of lists. In essence, for each simulation, the code ran the **stroke_simulation** function for each specified number of beds. This means that if there were $2000$ simulations and $25$ different numbers of beds (from 5 to 30), the `run_simulation` function ran a total of $50,000$ times $(2000 \times 25)$, each time with a different combination of simulation number and number of beds. The results of each of these simulations were collected into a list of data frames, which were then combined into a single data frame for further processing.

[^5]: For more details on lapply function:

    Bookdown: https://bookdown.org/rdpeng/rprogdatascience/loop-functions.html

    Pennsylvania State University: https://astrostatistics.psu.edu/su07/R/html/base/html/lapply.html

#### Results

In this section I will discuss the results from my simulation by visualizing the results.

I ran the simulation for $2000$ times for $25$ different number of beds (from $5-30$) for $350$ patients ( `run_simulation` function ran a total of $2000 \times 25 = 50000$ times, each time with the different combination of simulation number and number of beds).
...
In the Figure 4, we see the relationship between the number of beds and the percent of patients waiting to be admitted into the ward. As discussed, ideally we would expect to have less than 5% of patients waiting to be admitted into the ward at any given time -- this 5% rule for the percent of patients waiting, is an **average or overall target for the year**. The Figure 4 shows that the optimal number of beds for a stroke ward with 350 patients annually would be 10 beds, if percent of patients waiting to be admitted into the ward were to be taken into account alone.
...
In Figure 5, we see that utilization percentages at $11$ beds for $350$ patients annually is less than $70 \%$. Typically $0.80 (80 \%)$ is considered an optimal utilization rate for wards; this threshold is often cited as a balance between efficiency (having enough patients to justify the beds used) and capacity (having enough free beds to accommodate influxes of patients). @aiken2002hospital
...
Hence a $70\%$ bed utilization percentages would be considered fine for a stroke ward. As per this metric, the optimal number of beds in the ward would be $10$ at which the bed utilization percentage is $\sim 70\%$.
...
As per our analysis, the maximum queue length at the availability of 10 beds is 15 patients, let's calculate what percent of 350 does this make. When we calculate $\frac {15} {350} * {100} = 4.2\%$, it means that at the point of maximum demand, the percent of total patients for the year that were waiting was 4.2%. This is in line with our target of less than 5% of patients waiting on average.

However, it's important to remember that this is the peak demand and does not necessarily reflect the average wait time across the year. This means that during our simulation there was a time in the year when up to 15 patients were waiting to be admitted (4.2% of the annual total patients), but this doesn't mean that 4.2% of patients were always waiting.
...
For the arrival_rate, as the multiplying factor increased, we observed a consistent rise in utilization, percent_patients_waiting, and max_queue_length. With a 1.5 multiplier, the mean utilization was 0.7581, increasing steadily to 0.9783 at a 4.5 multiplier. This indicates that as more patients arrive, the healthcare system's resources become increasingly utilized. The waiting percentage of patients also increased from a mean of 15.684 to 57.50, while the max_queue_length ballooned from 54.89 to 201.2.

These changes show the strain on the healthcare system as the influx of patients surges.
...
In summary, the stroke simulation model devised in this investigation yields crucial understandings pertinent to the efficient management of hospital resources, particularly concerning stroke patient treatment. Remarkably, it was observed that the intricate interplay between the count of hospital beds, yearly patient influx, and bed occupancy durations, results in substantial effects on both patient wait times and bed utilization ratios.

The key value of individualized resource allocation was underscored by our model, as demonstrated through the adjustable stroke severity levels. This essential feature accentuates the notion that stroke cases differ in their demand for resources, a vital element to be considered in policy development. Additionally, the ability of our model to imitate various availability of hospital beds was instrumental in evaluating their impact on patient waiting periods and bed utilization rates.

The implications of this model in practical scenarios are profound. It offers an intuitive tool to policy makers and hospital management for efficient resource strategy formulation, based on live data and fluctuating patient load. More importantly, the model is inherently connected to the core objective of our study/project, which is to enhance the allocation of hospital resources for stroke patients, focusing on minimizing patient waiting durations and optimizing bed utilization rates.

Despite its advantages, our model has its inherent limitations.

It oversimplifies a complex reality, based on assumptions about patient arrival times, stroke severity distributions, and bed occupancy durations. The actual situation may stray from these assumptions. Instances like sudden patient surges due to epidemic situations or natural calamities aren't factored into the current model. Also, the model overlooks other potential chokepoints in stroke patient care, like the availability of healthcare staff or medical apparatus.

Future enhancements to this model could include its extension to a broader healthcare simulation, assimilating other elements of hospital resources and facilities. Including dynamic variables such as time-dependent arrival rates and fluctuating stroke severity levels could provide a more accurate representation of hospital operations. Further, future iterations could investigate the integration of machine learning methodologies for predicting patient loads and stroke severity distributions, thereby enhancing the decision-making process.

To encapsulate, our stroke simulation model aids in understanding and addressing the complicated task of hospital resource allocation for stroke patients. It paves the way towards more robust, dynamic, and inclusive healthcare simulation models. Despite its constraints, the model demonstrates significant potential to inform and impact hospital management strategies, eventually contributing to better patient care.
