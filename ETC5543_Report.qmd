...
In the R function `generate_length_of_stay`, the length of stay for each patient is calculated using this piecewise constant function. The function iterates through all patients, assigning a length of stay based on their stroke category:

1\. Retrieve the stroke category for the patient, denoted by `stroke_level`.

2\. Check if the bed occupation time for that category is a single value or a range of values.

3\. If it's a single value, assign the length of stay to that value: `length_of_stay[i] <- bed_occupation_time[[stroke_level]][1]`.

4\. If it's a range of values, randomly sample a value from that range and assign it as the length of stay: `length_of_stay[i] <- sample(bed_occupation_time[[stroke_level]][1]:bed_occupation_time[[stroke_level]][2], 1)`.
