...
The simulation model considered factors such as patient arrival rates, stroke severity probabilities, and bed occupation times. Patient arrival rates were estimated to be 1 patients/day, based on an annual patient load of \~350. The distribution of stroke severity was as follows:
...
For the purpose of this project at the *Monash Medical Research institute*, I have used the **simulation modelling** grounded in queue theory to represent a queuing system within a hospital setting. The main objective of this simulation was to simulate the flow of patients as they arrive, wait in line (queue) for an available bed, receive treatment, and exit the resource (making the bed available for the next patient in the queue to use).

### *Objectives and Significance*

#### **Objectives** {#sec-objectives}

The primary objective of this project was to develop *discrete event simulation* (DES) model that accurately captures the dynamics of the stroke ward at the Monash hospital. The goal was to ensure that the complex interactions between patients and beds are captured by the model so that this could be used to assess an optimal number of beds needed at the stroke ward.

In order to achieve these objectives an evaluation of different bed capacities on two key performance metrics: the percent of the patients waiting to be admitted into the ward and the utilization rates/percentages were considered.

The modek is intended to be used as a decision support tool by the hospital administrators and policymakers in optimizing bed allocation at the stroke ward.
...
length_of_stay <- function(total_patients, bed_occupation_time, stroke_levels) {
  set.seed(07052023)
...
length_of_stay<- length_of_stay(total_patients, bed_occupation_time, stroke_levels)
